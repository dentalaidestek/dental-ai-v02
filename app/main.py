...
async def upload_study_materials(
    ...
    try:
        with Session(engine, expire_on_commit=False) as s:
            ...
            for upload in selected:
                ...
                s.add(material)
                # ID'yi burada al; arka plan sadece yeni dosyaları indekslesin.
                s.flush()
                if material.id:
                    new_material_ids.append(int(material.id))
                added += 1
    ...
    # İndeksleme soru anında değil, yükleme bittikten sonra bir kez yapılır.
    if new_material_ids:
        background_tasks.add_task(
            _index_study_materials_background,
            user.id,
            course_id,
            new_material_ids,
        )
...
def _index_study_materials_background(
    owner_user_id: int,
    course_id: int,
    material_ids: list[int],
) -> None:
    """Index only newly uploaded materials after the upload response is sent."""
    try:
        with Session(engine, expire_on_commit=False) as session:
            for material_id in material_ids:
                ...
                if (
                    not material
                    or material.owner_user_id != owner_user_id
                    or material.course_id != course_id
                ):
                    continue
                try:
                    ensure_material_index(session, material)
                except StudyRAGError:
                    # Ask route can safely schedule a whole-course retry later.
                    continue
    except Exception:
        return


def _index_study_course_background(
    owner_user_id: int,
    course_id: int,
) -> None:
    """Recover missing legacy/new indexes without blocking a chat request."""
    try:
        with Session(engine, expire_on_commit=False) as session:
            materials = session.exec(
                select(StudyMaterial)
                .where(StudyMaterial.course_id == course_id)
                .where(StudyMaterial.owner_user_id == owner_user_id)
                .order_by(StudyMaterial.created_at.asc())
            ).all()
            if materials:
...
def _remember_study_exchange_background(
    ...
    """Optional semantic chat memory; disabled by default to save one embedding call."""
    try:
        with Session(engine, expire_on_commit=False) as session:
            remember_exchange(
                session,
...
def study_ai_ask(
    ...
    try:
        with Session(engine, expire_on_commit=False) as s:
            ...
            # Soru anında PDF hashleme/yeniden indeksleme yapma.
            # Yükleme sonrası arka plan indeksi hazır değilse hızlıca bilgi ver ve
            # aynı indeksi arka planda yeniden dene.
            if not course_index_ready(
                s,
                owner_user_id=user.id,
                course_id=course_id,
                materials=materials,
                ...
                background_tasks.add_task(
                    _index_study_course_background,
                    user.id,
                    course_id,
                )
                return JSONResponse({
                    "ok": False,
                    "error": "Ders notları hazırlanıyor. Biraz sonra tekrar deneyin.",
                }, status_code=503)
            ...
            # Yakın sohbet zaten history ile modele gidiyor. Ek semantic-memory
            # embedding çağrısı varsayılan olarak kapalıdır; istenirse ENV ile açılır.
