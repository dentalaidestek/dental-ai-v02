...
(function(){
  ...
  function toothFromPolygon(t,stats){
    ...
    g.position.y=-(1-side*side)*stats.depth*.34;
    g.userData.tooth=t;mesh.userData.tooth=t;g.add(mesh);return{g,mesh};
  }
  ...
  window.renderPatientPanoramicMesh=async function(){
    ...
    // Sinus geometry is intentionally withheld until a real contour is supplied.
    result.anatomy3d={mode:'panoramic_patient_mesh',rendered_teeth:teeth.length,patient_specific_depth:false,bone_contours_rendered:boneN,atlas_used:false,primitive_jaw_used:false};
    fit(jawScene,root);
