...
_TARGETS = [
    ...
    ValidationTarget("HORIZONTAL_BONE_LOSS", "composed_candidate", ("tvem_bone_loss", "panoreader_boneloss", "fdi", "crest_geometry"), True, "Generic bone-loss segmentation + validated crest orientation measurement."),
    ValidationTarget("VERTICAL_BONE_LOSS", "composed_candidate", ("tvem_bone_loss", "panoreader_boneloss", "fdi", "angular_defect_geometry"), True, "Generic bone-loss segmentation + localized angular-defect measurement."),
    ValidationTarget("FURCATION_BONE_LOSS", "train_required", ("zenodo14:FUR", "bone_loss"), True, "Zenodo annotated FUR class is available; train/resume a specialist rather than relabel generic bone loss."),
    ...
    ValidationTarget("PERI_IMPLANT_BONE_LOSS", "composed_candidate", ("IMPLANT", "bone_loss", "implant_cervical_geometry"), True, "Public study reports strong dedicated performance but releases no weights/data; validate composition first."),
